...
def run():
    ...
    'The person position'
    person_position_x = 2647.90
    person_position_y = 3267.36
    ...
    x_destination = 3058.91
    y_destination = 1788.62
    ...
    while traci.simulation.getMinExpectedNumber()>0:
        ...
        all_vehicles=np.arange(len(traci.vehicle.getIDList()))
        not_considered=['subway_L1:0.1', 'subway_L1:0.2', 'subway_L1:1.1', 'subway_L1:1.2']
        vehicles_filtred=all_vehicles[~np.isin(all_vehicles, not_considered)]
        if traci.simulation.getCurrentTime() == 100000:
            """
            A traffic demnad release at 300s
            """
            
            distance_person2cars = np.zeros(len(vehicles_filtred))
            'Find the nearest car'
            
            for idex_car in vehicles_filtred:
            ...
            id_closest_car = vehicles_filtred[index_min]
        ...
        if traci.simulation.getCurrentTime() > 100000 :
            if (id_closest_car in vehicles_filtred) and not person_in_car:
        ...
        if traci.simulation.getCurrentTime() > 100000 and person_in_car :
...
if __name__ == "__main__":
    ...
    sumoProcess = subprocess.Popen([sumoBinary, "-c", "../sumo/configuration.sumocfg", "--tripinfo-output", "tripinfo_1.xml", "--remote-port", str(PORT)])
    list_position = run()
    sumoProcess.wait()